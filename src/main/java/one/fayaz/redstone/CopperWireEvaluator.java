...
public abstract class CopperWireEvaluator {
    protected final CopperDustBlock wireBlock;

    protected CopperWireEvaluator(final CopperDustBlock wireBlock) {
    ...
    protected int getWireSignal(final BlockPos pos, final BlockState state) {
        return state.is(this.wireBlock) ? (Integer)state.getValue(CopperDustBlock.POWER) : 0;
    }
